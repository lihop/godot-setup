...
class Linux extends Platform {
  async preDownload() {
    ...
    await exec.exec(
      "sudo DEBIAN_FRONTEND='noninteractive' apt-get install -o=Dpkg::Use-Pty=0 -yqq pulseaudio xvfb x11-xserver-utils mesa-vulkan-drivers libwayland-client0 libwayland-cursor0 libwayland-egl1",
    );
    ...
    if (this.arch === "x86_32") {
      ...
      let packages =
        "pulseaudio xvfb x11-xserver-utils libasound2:i386 libpulse0:i386 libxcursor1:i386 libxinerama1:i386 libxrandr2:i386 libxi6:i386 libgl1:i386 libgles2:i386 libegl1:i386 libvulkan1:i386 libudev1:i386 libgbm1:i386 mesa-vulkan-drivers:i386 libwayland-client0:i386 libwayland-cursor0:i386 libwayland-egl1:i386";
    ...
    if (this.arch === "arm32") {
      ...
      core.info("Configuring ARM32 emulation safeguards...");
      ...
      core.exportVariable("GODOT_FORCE_RENDERING_DRIVER", "opengl3");
      core.exportVariable("MESA_GL_VERSION_OVERRIDE", "3.3");

      // ARM32 emulation safeguards
      core.exportVariable("GODOT_USE_THREADS", "false");
      core.exportVariable("MALLOC_CHECK_", "0");
      core.exportVariable("MALLOC_PERTURB_", "0");
      core.exportVariable("G_SLICE", "always-malloc");

      // Graphics and memory safeguards for ARM32 emulation
      core.exportVariable("MESA_NO_ERROR", "1");
      core.exportVariable("LIBGL_ALWAYS_SOFTWARE", "1");
      core.exportVariable("GALLIUM_DRIVER", "softpipe");
      core.exportVariable("GODOT_NO_VSYNC", "1");
      core.exportVariable("GODOT_PHYSICS_THREADS", "1");

      // Disable problematic features for ARM32 emulation
      core.exportVariable("GODOT_DISABLE_GLES3", "1");
      core.exportVariable("GODOT_DISABLE_VULKAN", "1");

      core.info("ARM32 emulation safeguards configured.");
    } else {
      try {
