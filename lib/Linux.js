...
class Linux extends Platform {
  async preDownload() {
    ...
    if (this.bits == 64) {
      ...
      await exec.exec(
        "sudo DEBIAN_FRONTEND='noninteractive' apt-get install -o=Dpkg::Use-Pty=0 -yqq pulseaudio xvfb x11-xserver-utils"
      );
    } else {
      await exec.exec("sudo dpkg --add-architecture i386");
      await exec.exec("sudo apt-get update -o=Dpkg::Use-Pty=0 -yqq");
      await exec.exec(
        "sudo DEBIAN_FRONTEND='noninteractive' apt-get install -o=Dpkg::Use-Pty=0 -yqq pulseaudio xvfb x11-xserver-utils libasound2:i386 libpulse0:i386 libxcursor-dev:i386 libxinerama-dev:i386 libxrandr-dev:i386 libxi-dev:i386 libgl-dev:i386 libudev-dev:i386"
      );
    }
