...
const DEFAULT_VERSION = "4.3-stable";
const PROCESS_ARCH_MAP = new Map([
  ["x64", "x86_64"],
  ["arm64", "arm64"],
  ["ia32", "x86_32"],
  ["arm", "arm32"],
...
class Platform {
  ...
  constructor() {
    ...
    // Ensure UTF-8 locale so Godot CLI doesn't crash on macOS runners.
    if (!process.env.LANG || process.env.LANG.trim() === "") {
      core.exportVariable("LANG", "en_US.UTF-8");
    }
    if (!process.env.LC_ALL || process.env.LC_ALL.trim() === "") {
      core.exportVariable("LC_ALL", "en_US.UTF-8");
    }
    ...
    const versionSegments = this.version.split(".");
    if (versionSegments.length === 3 && versionSegments[2] === "0") {
      // Normalize versions to match Godot's release naming (e.g. 'v4.0.0-stable' -> 'v4.0-stable').
      this.version = `${versionSegments[0]}.${versionSegments[1]}`;
    }

    this.arch = this._getArch();

    const version = semver.coerce(`${this.version}-${this.release}`);
    if (semver.lt(version, "4.0.0")) {
      throw new Error(
        `Setup only supports Godot 4.x and newer. Requested version 'v${this.version}-${this.release}' is not supported.`,
      );
    }

    core.info(
      `Detected version: v${this.version}-${this.release} ${this.arch} ${
        this.mono ? "Mono" : ""
      }`,
    );
  ...
  async download() {
    ...
    if (this.cache) {
      ...
      try {
        let cacheKey = await cache.restoreCache(
          [this.installDir],
          `${this.downloadUrl}${
            this.exportTemplates ? " " + this.exportTemplatesUrl : ""
          }`,
        );
        restoredFromCache = !!cacheKey;
      } catch (error) {
    ...
    if (needsCache && !restoredFromCache) {
      core.info(
  ...
  _getArch() {
    let arch = core.getInput("arch", {
      default: PROCESS_ARCH_MAP.get(process.arch),
    });

    // Handle empty string inputs by falling back to default
    if (!arch || arch.trim() === "") {
      arch = PROCESS_ARCH_MAP.get(process.arch);
    }

    const runnerArch = this._getRunnerArch();

    if (
      !(this._getRunnerArchCompatibilityMap().get(runnerArch) || []).includes(
        arch,
      )
    ) {
      throw new Error(
        `Architecture '${arch}' is not compatible with runner architecture '${runnerArch}'`,
      );
    }
  ...
  _getRunnerArch() {
    return PROCESS_ARCH_MAP.get(process.arch);
  }

  _getRunnerArchCompatibilityMap() {
    return new Map([
      ["x86_64", ["x86_64", "x86_32"]],
      ["arm64", ["arm64", "arm32"]],
      ["x86_32", ["x86_32"]],
      ["arm32", ["arm32"]],
    ]);
  ...
  _getBits() {
    switch (this.arch) {
      ...
      case "arm32":
        return 32;
      default:
        throw new Error(`Unknown architecture '${this.arch}'`);
    }
  }
